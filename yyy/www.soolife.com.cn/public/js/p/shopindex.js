$(function(){
    $('.gree_up').on('click',function(){
        $('.lj_datum').hide();
        $('.lj_emboitement').show();
    });
    $('.lj_nexter_up').on('click',function(){
        $('.uploading').hide();
        $('.lj_datum').show();
    });
    $('.lj_sub_up').on('click',function(){
        $('.bargain').hide();
        $('.uploading').show();
    });
	// 线上服务单选
    ...
    // 点击协议选项
    $(".choo").on('click',function(){
        var _src = $(this).attr("src");
        var _src1 = "/public/img/sop/Group21.png";
        if(_src == _src1 ){
            if (fisrt_data()){
                $('.total_money').html(total_money/10000);
                $('.total_coin').html(total_coin/10000);
                $(this).attr("src",_src2);
            }
        }else{
            $(this).attr("src",_src1);
    ...
    $('.lj_next').on('click',function(){
        if(fisrt_data()){
            ...
            // 协议是否勾选
            var choo_src = $('.choo').attr("src");
            if (choo_src != '/public/img/sop/Group19.png') {
                bootbox.alert("请勾选协议选项");return false;
            }
            $('.lj_emboitement').hide();
            $('.lj_datum').show();
        }
    ...
    $('.gree').on('click',function(){
        ...
        if (basic_information.company_name == ''){bootbox.alert('公司名称必填');return;}
        basic_information.region_id = $('.county').find('option:selected').val().trim();
        basic_information.address = $('input[name=address]').val().trim();
        if (basic_information.address == ''){bootbox.alert('公司地址必填');return;}
        basic_information.industry = $('input[name=industry]').val().trim();
        if (basic_information.industry == ''){bootbox.alert('所属行业必填');return;}
        basic_information.scope = $('input[name=scope]').val().trim();
        if (basic_information.scope == ''){bootbox.alert('经营范围必填');return;}
        basic_information.capital = $('input[name=capital]').val().trim();
        if (basic_information.capital == ''){bootbox.alert('注册资本必填');return;}
        basic_information.corporation = $('input[name=corporation]').val().trim();
        if (basic_information.corporation == ''){bootbox.alert('法人姓名必填');return;}
        basic_information.identitycard = $('input[name=identitycard]').val().trim();
        if (basic_information.identitycard == ''){bootbox.alert('身份证号必填');return;}
        if (!isIdCardNo(basic_information.identitycard)){bootbox.alert('身份证号不正确');return;}
        basic_information.business_card = $('input[name=business_card]').val().trim();
        if (basic_information.business_card == ''){bootbox.alert('营业执照号必填');return;}
        basic_information.establish = $('input[name=establish]').val().trim();
        if (basic_information.establish == ''){bootbox.alert('公司成立日期必填');return;}
        basic_information.begin_date = $('input[name=begin_date]').val().trim();
        if (basic_information.begin_date == ''){bootbox.alert('营业开始时间必填');return;}
        basic_information.end_date = $('input[name=end_date]').val().trim();
        if (basic_information.end_date == ''){bootbox.alert('营业终止时间必填');return;}
        basic_information.company_tel = $('input[name=company_tel]').val().trim();
        if (basic_information.company_tel == ''){bootbox.alert('公司联系电话必填');return;}
        if (!isPhone(basic_information.company_tel)){bootbox.alert('公司联系电话不正确');return;}
        basic_information.company_remark = $('input[name=company_remark]').val().trim();
        if (basic_information.company_remark == ''){bootbox.alert('备注内容必填');return;}
        basic_information.linkman = $('input[name=linkman]').val().trim();
        if (basic_information.linkman == ''){bootbox.alert('联系人姓名必填');return;}
        basic_information.phone = $('input[name=phone]').val().trim();
        if (basic_information.phone == ''){bootbox.alert('联系人电话必填');return;}
        if (!isPhone(basic_information.phone)){bootbox.alert('联系人电话不正确');return;}
        basic_information.email = $('input[name=email]').val().trim();
        if (basic_information.email == ''){bootbox.alert('联系人邮箱必填');return;}
        if (!mail(basic_information.email)){bootbox.alert('联系人邮箱不正确');return;}
        basic_information.duty = $('input[name=duty]').val().trim();
        if (basic_information.duty == ''){bootbox.alert('联系人职务必填');return;}
        basic_information.facsimile = $('input[name=facsimile]').val().trim();
        if (basic_information.facsimile == ''){bootbox.alert('传真必填');return;}
        if (!isFax(basic_information.facsimile)){bootbox.alert('传真号码不正确');return;}
        basic_information.urgency_linkman = $('input[name=urgency_linkman]').val().trim();
        if (basic_information.urgency_linkman == ''){bootbox.alert('紧急联系人姓名必填');return;}
        basic_information.urgency_phone = $('input[name=urgency_phone]').val().trim();
        if (basic_information.urgency_phone == ''){bootbox.alert('紧急联系人电话必填');return;}
        if (!isPhone(basic_information.urgency_phone)){bootbox.alert('紧急联系人电话不正确');return;}
        $('.lj_datum').hide();
        $('.uploading').show();
    });
    // 点击上传图片页下一步
    ...
    function validateForm(){
        return $('#form_all_data').validate({
            ...
            messages: {
                ...
                paperbrandsales: {
                    required: "请上传品牌销售授权证明",
                    checkPicSize: "请上传大小在150k以下的图片",
                    checkPicType:"仅支持JPG、GIF、PNG格式的图片"
                },
                paperquality: {
                    required: "请上传质检报告或产品质量合格证明",
                    checkPicSize: "请上传大小在150k以下的图片",
                    checkPicType:"仅支持JPG、GIF、PNG格式的图片"
                },
            },
    ...
    $('.lj_sub').on('click',function(){
        ...
        var form_all_data = document.getElementById("form_all_data");
