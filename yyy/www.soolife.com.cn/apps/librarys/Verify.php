...
class Verify extends Component {
	...
	protected $config = array(
		'seKey' => 'ILOVETONY.COM', // 验证码加密密钥
		'codeSet' => '23456789abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY', // 验证码字符集合
		'numericSet' => '1234567890', // 验证码字符集合
	...
	public function check($code, $key = '') {
		...
		$redis_code = $this->vcode_redis($key);
		return $redis_code == $code ? true : false;
	}
	...
	public function entry($id = '') {
		...
		// 验证码使用随机字体
		$ttfPath = $this -> font_path . ($this -> useZh ? 'zhttfs' : 'ttfs') . '/';
		...
		}else if ($this -> useNumeric) {// 数字
			for ($i = 0; $i < $this -> length; $i++) {
				$code[$i] = $this -> numericSet[mt_rand(0, strlen($this -> numericSet) - 1)];
				$codeNX += mt_rand($this -> fontSize * 1.2, $this -> fontSize * 1.6);
				imagettftext($this -> _image, $this -> fontSize, mt_rand(-20, 20), $codeNX, $this -> fontSize * 1.6, $this -> _color, $this -> fontttf, $code[$i]);
			}
		}else {
			for ($i = 0; $i < $this -> length; $i++) {
	...
	/**
	* 将验证码信息存入redis
	* @param name , 唯一标识 | value ,验证码值
	* @author Jinlong_Xie <soosim@qq.com>
	* @date 2016-08-11 11:49:36
	*/
	private function vcode_redis($key,$value='',$lifetime=300)
	{
		if(empty($key)) return false;
		$redis = new WebRedis();
		$key = 'member:vcode:'.$key;
		if(empty($value))
		{
			$res = $redis->read($key,$this->db);
			return $res;
		}else{
			$redis->remove($key,$this->db);
			$redis->write_simple($key,json_encode($value),$this->db,$lifetime);
		}
	}
}
