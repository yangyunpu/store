...
function dealHref(){
	var arr = [];
	var _href_opsition = window.location.href.indexOf('hreecate');
	var _hreflength = window.location.href.length;
	var href_p = _href.substring(_href_opsition+4,_hreflength);
	return href_p;
}; 
...
// 	if(urlb[i].indexOf('firstcode')==0||urlb[i].indexOf('twocode')==0||urlb[i].indexOf('threecode')==0||urlb[i].indexOf('keyword')==0||urlb[i].indexOf('csstag')==0){
// 		yesUrl+= urlb[i]+"&";
// 	};
// };
var str = "";
...
$('.surebtn').click(function(){
	...
	//var sss = $.base64.encode(_span_str);
	//var brand_code = $.base64.encode(_brand);

	_specparam_str=_specparam.join(',');
	var backHref = "/newcategory/threecate.html?firstcode=" + firstcode + "&twocode=" + twocode + "&threecode=" + threecode +"&keyword="+keyword+"&brand_id="+brand_code+"&shop_id="+_shop+"&countries="+_countries+"&specs="+_specparam+"&_kai="+_kai+"&_jie="+_jie+"&_span_str="+sss;
