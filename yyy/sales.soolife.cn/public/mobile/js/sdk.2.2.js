...
//连接App 和web之间的关系
	...
	connectWebViewJavascriptBridge(function(bridge) {
		...
		bridge.registerHandler("soolife_shopping_js_Echo", function(data, responseCallback) {});
	})
...
window.appSDK = (function(){
	var invoke = function(bodyDict) {
		if(isiOS) {
			...
			window.WebViewJavascriptBridge.callHandler('android Echo', bodyDict,
	...
	return {
		...
		// pay center
		openPayCenter: function(orderArr,url) {//支付中心
			toolFunc({
				"action": "open",
				"object": "paycenter",
				"param": {
...
var webSdk = {
		...
		getTokenObj1	   :"get-token-value1",
		// getTokenUrl		   : ['http://sales.test.soolife.cn/hot/onebuy.html'],
		getTokenUrl		   : [null],
		shareObj 	       : "open-share",
		shareUrl 	       : [null],
		//下面功能正在开发中...没有调用....
		...
		payCenterObj       : "open-pay-center",
		payCenterUrl       : 'http://orders.test.soolife.cn/m/order/orderpay.html',
		...
		getTokenValue:function(obj,href){//获得token值；
			if(!isApp) return;
			var that = this;
					...
					appSDK.getToken({
						success:function(argument){
							if (argument == "" || argument == null) {
								...
								document.cookie="m_token="+argument;
								$("[name|='mobile']").val("");
								$("[name|='address']").val("");
								$("[name|='name']").val("");
								$("#region").val("");
		...
		getTokenValue1:function(obj,href){//获得token值；
			if(!isApp) return;
			var that = this;
			var btn = document.getElementsByClassName(this.getTokenObj1);//分享当前页web
			for(var i = 0;i < btn.length;i++){
				var _href = that.getTokenUrl[0];
				appSDK.getToken({
					success:function(argument){
		...
		openPayCenter:function(){// 支付中心
			var that = this;
			var objBtn = document.getElementsByClassName(this.payCenterObj);
			for(var i = 0;i < objBtn.length;i++){
				objBtn[i].onclick = function(){
					 var _href = that.payCenterUrl ;

					appSDK.openPayCenter(orderArr,_href);
				};
			};
		}
};//<webSdk--end>
...
function BaseSdk(){};
	...
	BaseSdk.prototype.openPayCenter = function(obj,objPara){//支付中心
		var obj = document.querySelectorAll(obj);
		for(var i = 0;i < obj.length;i++){
			obj[i].onclick = function(){
				var _href = null;
