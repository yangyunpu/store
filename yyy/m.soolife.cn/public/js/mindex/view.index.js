    ...
    window.onscroll=function(){
        ...
        if(a+e == c){
            if( pageData.length>0){
                var partData = pageData.splice(0,8);//页面展示2个
                console.log(pageData)
                console.log(partData)
                append_data(partData);
            }else{
                index++;
...
// 热门搜索自动补全
    $(".search_auto").bind("input propertychange",function(event) {
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                if(data != undefined){
                    for(var i=0; i<data.length; i++){
                        // var _li = "<li class='hot'>"+data[i]['wd']+"</li>";
                        if(data[i] != undefined){
                            var _li ='<li>'
                                    +    '<a class="set_history" href="/newcategory/threecate.html?firstcode=&amp;keyword='+data[i]['wd']+'&amp;csstag=9"><span class="hot">'+data[i]['wd']+'</span></a>'
                                    +    '</li>'
                        }
