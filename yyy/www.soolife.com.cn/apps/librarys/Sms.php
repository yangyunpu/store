...
use Soolife\Member\Services\AccountService;
...
class Sms extends Component{
   ...
   public function phone($phone)
	 ...
	 * 注册成功发送邮箱
	 * @access public
	 * @author Luo Qing 2016-07-06
	 * POST v1/email/register
	 * **/
    public function email($name,$nickname,$emails,$member_id){
        $emailurl = "/v1/email/register";
        $email= array();
        $email['username'] = $name;
        $email['nickname'] = $nickname;
        $email['email']    = $emails;
        $email['memberId'] = $member_id;
        $account = new AccountService();
        $result  = $account -> emailregister($email);
    }
    ...
    */
    public function send_msg($tele,$type){
        ...
        $url = '/sms/'.$type.'/'.$tele;
        $code = $this->curl->get_request($url,'php_api');
        if($code == 200){
            $this->limit_send($tele,100);
