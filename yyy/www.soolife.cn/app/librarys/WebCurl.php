...
class WebCurl extends Component {
	 ...
	 * 设置API认证的账号与密码
	 */
	public function get_auth($method) {
		if(in_array($method,array('v2_api','java_api')))
		{
			$appid = $this -> config -> $method -> app_id;
			$appkey = $this -> config -> $method -> app_key;
		}
		else
		{
	...
	public function get_url($url,$method) {
		if(in_array($method,array('php_api','v2_api')))
		{
			$host = $this-> config-> $method-> url;
		}elseif($method == 'java_api'){
			$host = $this -> config -> java_api -> url;
		}else{
	 ...
	 * @param $data Array 数据
	 */
	public function upload_request($url, $files, $data,$method=1) {
		$data = array_merge($files, $data);
		$post = array(CURLOPT_POSTFIELDS => $data);
		return $this -> send($url, $post,'',$method);
	}
	...
	public function send($url, $post = '', $len = 0,$method) {
		...
		$options[CURLOPT_USERPWD] = $this -> get_auth($method);
